<template>
  <!--REGISTER ADDRESS-->
  <section class="registerAddress mt-3">
    <v-container class="px-15">
      <v-row>
        <v-col cols="12" md="12">
          <h1 class="dark--text">Portfolio</h1>
          <nuxt-link
            to="/admin/portfolio/adicionar"
          ...
          >
            <div class="a-box first-desktop-address-tile">
              <div class="a-box-inner a-padding-extra-large">
                ...
                <v-btn color="primary" class="h2">Adicionar Novo</v-btn>
              </div>
            </div>
          </nuxt-link>
        </v-col>
        ...
        <v-col
          cols="12"
          md="12"
          v-for="(address, index) in clients"
          :key="address._id"
        >
          <v-card flat>
            <v-container>
              <v-row class="address-result">
                ...
                <v-col cols="12" sm="12" md="2" lg="2" class='actions'>               
                  <div>
                    <v-btn fab small color="warning" :to="`/painel-de-controle/endereco/${address._id}`">
                      <v-icon>edit</v-icon>
                    </v-btn>
                    <v-btn
                      color="error"
...
export default {
  ...
  async asyncData({ $axios }) {
    try {
      let addresses = await $axios.$get("/api/clients");
      return {
